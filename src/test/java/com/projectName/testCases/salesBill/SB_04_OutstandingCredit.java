...
public class SB_04_OutstandingCredit extends WebDriverInit implements DataConstantsSalesBill, Validations {
    ...
    @Test(priority = 3, dependsOnGroups = "ReturnMedicine")
    public void verifyReturnOutstandingCredit() {
        try {
            testCaseLog("verifyReturnOutstandingCredit");
            map.putAll(getTestDataWorkbook(DATA_RETURN_SALES_BILL_TWO_MED));

            salesOrderController.getDeleteSalesInvoice(sideMenuController, getData(map, PATIENT_NAME, 0), getData(map, DOCTOR_NAME, 0), getData(map, PAYMENT_MODE, 0),
                    getData(map, DATE, 0), getData(map, TOTAL_AMOUNT, 0), getData(map, SALES_STATUS, 0));
    ...
    @Test(dependsOnMethods = "verifyReturnOutstandingCredit")
    public void verifySalesBill() {
        ...
        salesOrderVerification.verifySalesBill(tabCount, getData(map, PATIENT_NAME, 0), getData(map, DOCTOR_NAME, 0),
                getData(map, PAYMENT_MODE, 0), getData(map, TOTAL_AMOUNT, 0), getData(map, SALES_DISCOUNT_RUPEES, 0),
