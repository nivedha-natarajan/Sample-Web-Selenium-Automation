...
public class SB_03_ReturnOneMedicine extends WebDriverInit implements DataConstantsSalesBill, Validations {
    ...
    @Test(priority = 2)
    public void verifyReturnMedicine() {
        try {
            testCaseLog("verifyReturnMedicine");
            map.putAll(getTestDataWorkbook(DATA_RETURN_SALES_BILL_ONE_MED));

            salesOrderController.getDeleteSalesInvoice(sideMenuController, getData(map, PATIENT_NAME, 0), getData(map, DOCTOR_NAME, 0), "", getData(map, DATE, 0), "", getData(map, SALES_STATUS, 0));
    ...
    @Test(dependsOnMethods = "verifyReturnMedicine", groups = "ReturnMedicine")
